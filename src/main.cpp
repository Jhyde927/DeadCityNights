...
void Dig(){
    ...
    if (player.position.x > 1860 && player.position.x < 1880 && !player.hasPills && gameState == CEMETERY) {
       ...
       // Draw the icon at x, y
        if (!inventory[i].empty()) { //inventory buttons are all done in the same for loop we use to draw it. Consider abstracting this somehow. 
            ...
            if (inventory[i] == "Badge"){
                ...
                bool hovered = CheckCollisionPointRec(mousePosition, badgeBounds);
                bool selected = globalState.selectedSlot == i;
                bool clicked = IsMouseButtonPressed(MOUSE_BUTTON_LEFT) || 
                               IsGamepadButtonPressed(0, GAMEPAD_BUTTON_RIGHT_FACE_DOWN);

                if ((hovered || selected) && clicked) {
                    globalState.showBadge = !globalState.showBadge;
                }
            }
            ...
            if (inventory[i] == "whiskey"){
                ...
                bool hovered = CheckCollisionPointRec(mousePosition, whiskeyBounds);
                bool selected = globalState.selectedSlot == i;
                bool clicked = IsMouseButtonPressed(MOUSE_BUTTON_LEFT) || 
                   IsGamepadButtonPressed(0, GAMEPAD_BUTTON_RIGHT_FACE_DOWN);

                if ((hovered || selected) && clicked && player.hasWhiskey) {
                    inventory[i].clear();
                    player.currentHealth = player.maxHealth;
            ...
            if (inventory[i] == "pills"){ // Click on pills to take them 
                ...
                bool hovered = CheckCollisionPointRec(mousePosition, pillBounds);
                bool selected = globalState.selectedSlot == i;
                bool clicked = IsMouseButtonPressed(MOUSE_BUTTON_LEFT) || 
                               IsGamepadButtonPressed(0, GAMEPAD_BUTTON_RIGHT_FACE_DOWN);

                if ((hovered || selected) && clicked && player.hasPills && player.currentHealth < player.maxHealth) {
                    player.currentHealth = player.maxHealth;
...
void CheckBulletPlayerCollisions() {
    ...
    for (int i = 0; i < MAX_BULLETS; ++i) {
        Bullet& b = bullets[i];
        if (!b.isActive || !b.laser) continue;

        if (player.CheckHit(b.previousPosition, b.position, bulletSize)) {
...
void CheckLaserNPCCollisions(std::vector<NPC>& collidingNPCs) {
    ...
    for (int i = 0; i < MAX_BULLETS; ++i) {
        ...
        for (NPC& npc : collidingNPCs) {
            if (!npc.isActive || npc.robot) continue;

            if (npc.CheckHit(b.previousPosition, b.position, laserSize)) {
                b.isActive = false;
...
void CheckBulletNPCCollisions(std::vector<NPC>& npcs) {
    for (int i = 0; i < MAX_BULLETS; ++i) {
        ...
        for (NPC& npc : npcs) {
            if (!npc.isActive || npc.isDying || b.hitNPCs.find(&npc) != b.hitNPCs.end()) continue;

            if (!npc.CheckHit(b.previousPosition, b.position, b.size)) continue;

            b.hitNPCs.insert(&npc);

            if (b.raygun && b.health > 0) {
                --b.health;
                npc.TakeDamage(b.damage);

                if (b.health <= 0) {
                    b.isActive = false;
                    b.hitNPCs.clear();
                    // Let others still get hit this frame
                }
            } else {
                npc.TakeDamage(b.damage);
                b.isActive = false;
                b.hitNPCs.clear();
                break; // Stop after one hit for normal bullets
            }
        }
    }
}
...
;        globalState.fortuneTimer -= GetFrameTime();
    ...
    if (globalState.overLiquor
        && globalState.showLiquor
        && player.money >= 100
        &&  GuiButton((Rectangle){ screen_pos.x+16, screen_pos.y-64, 64,48 }, "BUY"))
    {
...
void RenderCemetery() {
   ...
   if (player.position.x > carMin && player.position.x < carMax && !globalState.spawning_zombies) {
        globalState.over_car = true;
        phrase = "UP TO ENTER";
        globalState.show_dbox = true;
        globalState.dboxPosition = player.position;
...
void RenderLot(){
    ...
    for (NPC& hobo : hobos) {
        ...
        if (!hobo.interacting) continue;

        if (globalState.firstHobo) {
            globalState.firstHobo = false;
            globalState.drawShovel = true;
            if (player.hasGun) {
                globalState.raiseZombies = true;
            }
        }
...
void RenderPark(){
    ...
    for (NPC& npc : ParkNpcs) {
        if (npc.MiB) {
            if (globalState.hasCemeteryKey && globalState.mibTimer > 0) {
                npc.Update();
                npc.Render();
                npc.ClickNPC();
                ...
                if (npc.interacting) {
                    ...
                    globalState.mibTimer -= GetFrameTime();
        ...
        if (!npc.isActive && npc.CanSpawnZombie) {
            npc.CanSpawnZombie = false;
            spawnZombie(npc.position);
        }
    }
...
void RenderPenthouse()
{
    ...
    Vector2 pentPos = {1800, 648};
    if (elevators[0].position != pentPos)
        elevators[0].position = pentPos;

    if (globalState.playAlarm && !SoundManager::getInstance().IsSoundPlaying("alarm"))
        PlaySound(SoundManager::getInstance().GetSound("alarm"));

    if (player.position.x > 3080 && player.position.x < 3100 && globalState.bossDefeated && !globalState.playAlarm) {
        phrase = "Press Up to Initiate\n\nSelf Destruct Sequence";
        globalState.overSDconsole = true;
        showDBOX();
    }

    if (player.position.x > 1877 && player.position.x < 1897) {
        phrase = "Call Elevator";
        globalState.over_Ebutton = true;
        showDBOX();
    }

    if (player.position.x > 1824 && player.position.x < 1844 && elevators[0].isOpen) {
        phrase = "Up to Enter";
        globalState.over_elevator = true;
        showDBOX();
    }

    if (player.position.x > 3276 && player.position.x < 3296 && globalState.bossDefeated && globalState.playAlarm) {
        phrase = "Call Elevator";
    ...
    float px = camera.target.x;
    DrawTexturePro(resources.penthouseBackground, {0, 0, (float)resources.penthouseBackground.width, (float)resources.penthouseBackground.height},
        {-700 + px * 0.8f, 0, (float)resources.penthouseBackground.width, (float)resources.penthouseBackground.height}, {0, 0}, 0, WHITE);
    DrawTexturePro(resources.penthouseMidBack, {0, 0, (float)resources.penthouseMidBack.width, (float)resources.penthouseMidBack.height},
        {-300 + px * 0.7f, 0, (float)resources.penthouseMidBack.width, (float)resources.penthouseMidBack.height}, {0, 0}, 0, WHITE);
    DrawTexturePro(resources.penthouseMidground, {0, 0, (float)resources.penthouseMidground.width, (float)resources.penthouseMidground.height},
        {px * 0.6f, 0, (float)resources.penthouseMidground.width, (float)resources.penthouseMidground.height}, {0, 0}, 0, WHITE);
    DrawTexturePro(resources.penthouseForeground, {0, 0, (float)resources.penthouseForeground.width, (float)resources.penthouseForeground.height},
        {512, 0, (float)resources.penthouseForeground.width, (float)resources.penthouseForeground.height}, {0, 0}, 0, WHITE);
    ...
    HandleGrenades();
    if (!player.hasArmor)
        DrawItem({1634, 700}, "vest");

    for (auto& monitor : monitors)
        if (monitor.scene == gameState) DrawMonitor(monitor);

    for (auto& console : consoles)
        if (console.scene == gameState) DrawConsole(console);
    ...
    if (!globalState.usingController || player.enter_car) {
        if (player.hasGun)
            DrawTexture(IsMouseButtonDown(MOUSE_BUTTON_RIGHT) ? resources.reticle : resources.handCursor, mousePosition.x, mousePosition.y, WHITE);
        else
    ...
    if ((player.hasGun || player.hasShotgun || player.hasMac10) && !player.enter_car)
        DrawHUD(player);
}
...
// Position constants
const Vector2 LOBBY_ELEVATOR_POS = {2446, 648};
const Vector2 MEDKIT_X_RANGE = {2031, 2051};
const Vector2 EXIT1_X_RANGE = {2750, 2770};
const Vector2 EXIT2_X_RANGE = {1310, 1330};
const Vector2 CALL_ELEVATOR_X_RANGE = {2519, 2539};
const Vector2 ENTER_ELEVATOR_X_RANGE = {2468, 2488};
...
void RenderLobby() {
    ...
    if (elevators[0].position.x != LOBBY_ELEVATOR_POS.x || elevators[0].position.y != LOBBY_ELEVATOR_POS.y) {
        elevators[0].position = LOBBY_ELEVATOR_POS;
    }

    float px = player.position.x;

    if (InRange(px, MEDKIT_X_RANGE.x, MEDKIT_X_RANGE.y)) {
    ...
    if (InRange(px, EXIT1_X_RANGE.x, EXIT1_X_RANGE.y) || InRange(px, EXIT2_X_RANGE.x, EXIT2_X_RANGE.y)) {
        globalState.over_exit = true;
        phrase = "UP TO EXIT";
        globalState.show_dbox = true;
        globalState.dboxPosition = player.position;
    ...
    if (InRange(px, CALL_ELEVATOR_X_RANGE.x, CALL_ELEVATOR_X_RANGE.y)) {
        globalState.over_Ebutton = true;
        phrase = "Call Elevator";
        globalState.show_dbox = true;
        globalState.dboxPosition = player.position;
    ...
    if (InRange(px, ENTER_ELEVATOR_X_RANGE.x, ENTER_ELEVATOR_X_RANGE.y) && elevators[0].isOpen) {
        globalState.over_elevator = true;
        phrase = "Up to Enter";
        globalState.show_dbox = true;
        globalState.dboxPosition = player.position;
    ...
    if (globalState.drunk) BeginShaderMode(shaders.glowShader2);

    DrawTexturePro(resources.subwayBackground,
                   {0, 0, (float)resources.subwayBackground.width, (float)resources.subwayBackground.height},
                   {0, 0, (float)resources.subwayBackground.width, (float)resources.subwayBackground.height},
                   {0, 0}, 0.0f, WHITE);

    DrawTexturePro(resources.LobbyForeground,
                   {0, 0, (float)resources.LobbyForeground.width, (float)resources.LobbyForeground.height},
                   {64, 0, (float)resources.LobbyForeground.width, (float)resources.LobbyForeground.height},
                   {0, 0}, 0.0f, WHITE);
    ...
    if (AreAllNPCsDeactivated(lobbyMibs) && globalState.can_spawn_robots && !globalState.robotsTriggered) {
        globalState.can_spawn_robots = false;
        globalState.robotsTriggered = true;
    ...
    for (NPC& robot : lobbyRobots) {
        ...
        robot.Render();
        robot.ClickNPC();
        if (globalState.globalAgro) robot.agro = true;
        if (robot.agro) {
            robot.destination = player.position;
            Flocking(lobbyRobots);
        }
    ...
    for (NPC& mib : lobbyMibs) {
        if (!mib.isActive) continue;
        mib.Update();
        mib.Render();
        mib.ClickNPC();
        if (globalState.globalAgro) mib.agro = true;
    ...
    for (NPC& zombie : zombies) {
        zombie.isActive = (zombie.scene == gameState);
    ...
    if (player.hasGun) {
        if (!globalState.usingController || player.enter_car) {
            DrawTexture(IsMouseButtonDown(MOUSE_BUTTON_RIGHT) ? resources.reticle : resources.handCursor,
                        mousePosition.x, mousePosition.y, WHITE);
        }
    } else {
        if (!globalState.usingController || player.enter_car) {
            DrawTexture(resources.handCursor, mousePosition.x, mousePosition.y, WHITE);
    ...
    if ((player.hasGun || player.hasShotgun) && !player.enter_car) {
        DrawHUD(player);
    }
}
...
void cleanup(SoundManager &soundManager) {
...
auto main() -> int {
    ...
    while (!WindowShouldClose() && !globalState.quitRequested) {
        updateGame(currentPauseState, targetTexture, vignetteTexture, finalTexture, pauseTexture, destRect);
    }
