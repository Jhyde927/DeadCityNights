...
uniform float glowThreshold; // Threshold above which pixels will glow
uniform float glowIntensity; // Intensity of the glow
uniform vec2 resolution;     // Resolution of the texture (render texture size)
...
void main() {
    ...
    if (luminance > glowThreshold) {
        ...
        vec4 glow = vec4(0.0);
        float total = 0.0;
        vec2 texelSize = 1.0 / resolution; // Size of one texel in texture coordinates

        // Simple blur kernel (3x3)
        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                vec2 offset = vec2(float(x), float(y)) * texelSize * glowIntensity;
                vec2 samplePos = uv + offset;
                glow += texture(texture0, samplePos);
                total += 1.0;
            }
        }
        glow /= total;
